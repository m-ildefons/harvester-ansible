...
DOCUMENTATION = r'''
module: rancher.get_kubeconfig

short_description: Fetch the kubeconfig file from a Rancher cluster
...
RETURN = r'''
kubeconfig:
    description: The kubeconfig received from the Rancher cluster
    type: str
    returned: always:
    sample:
'''
...
def main():
    ...
    try:
        ...
        resp = api.post("v3/clusters/local", params=params)
        assert resp.status_code == 200
    except AssertionError:
        module.fail_json(msg='failed to fetch kubeconfig')
    else:
